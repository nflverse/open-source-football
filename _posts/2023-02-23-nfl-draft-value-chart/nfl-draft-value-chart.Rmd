...
```{r, echo = FALSE}
 raw_otc <- read_html("https://overthecap.com/draft-trade-value-chart") %>%
  ...
  jj <- bind_rows(
    ...
    mutate(
      osf_surplus = surplus / dplyr::first(surplus),
...
* Lions receive: No. 12 (169), No. 46 (124). Total: 293
* Vikings receive: No. 32 (139), No. 34 (146), No. 66 (96). Total: 381
...
* Jets receive: No. 14th (168), No. 143 (35). Total: 203
* Vikings receive: No. 23 (154), No. 66 (96), No. 86 (71). Total: 321
* Difference: 118, or the value of **pick No. 5** (which is the same as the surpluse value of pick 51)
...
* Seahawks receive: 2022 pick 109 (1.09 * .9)
* Jets receive: 2021 pick 23 (3.15), 2021 pick 86 (1.46), 2022 pick 10 (3.26 * .9)
* Difference: 3.15 + 1.46 + (3.26 * .9) - (1.09 * .9) = **6.6 percent of the salary cap** per year over 4 years
...
table_data <- df %>%
  filter(name %in% c("osf_surplus", "surplus"), !is.na(value)) %>%
  pivot_wider(names_from = "name", values_from = "value") %>%
  mutate(
...
t1 <- table_data[1:55,]
t2 <- table_data[56:110,]
t3 <- table_data[111:165,]
t4 <- table_data[166:220,]


table <- bind_cols(t1, t2, t3, t4) %>%
  gt::gt()  %>% 
  cols_label(
    draft_overall...1 = "Pick", draft_overall...4 = "Pick", draft_overall...7 = "Pick", draft_overall...10 = "Pick",
    osf_surplus...2 = "Points", osf_surplus...5 = "Points", osf_surplus...8 = "Points", osf_surplus...11 = "Points",
    surplus...3 = "APY", surplus...6 = "APY", surplus...9 = "APY", surplus...12 = "APY"
  ) %>% 
  ...
  tab_style(
    ...
    tab_footnote(
    footnote = "APY as percent of salary cap",
