...
library(tidyverse)
library(nflfastR)
pbp <- data.table::fread("../../../Darkweb/pbp_2010-2019.csv") %>% filter(season == 2019)
roster <-
...
Next, we have a couple of players that are in our Top 30 from last year that have changed teams from where our roster lists them or the data is missing. 
We can quickly fill in a couple missing entries like below.
...
summary <-
  ...
  filter(!is.na(cpoe)) %>%
  group_by(lastName, posteam, firstName, passer_player_id) %>%
  summarise(plays = n()) %>%
  ungroup() %>%
  arrange(desc(plays)) %>%
  head(30) %>%
  ...
  left_join(
    roster %>% filter(team.season == season) %>% mutate(firstInit = str_extract(teamPlayers.firstName, "\\w")) %>% select(name = teamPlayers.displayName, firstInit, teamPlayers.lastName, team.abbr, teamPlayers.headshot_url),
    by = c("lastName" = "teamPlayers.lastName", "firstName" = "firstInit")
  ) %>%
...
Lastly, we can kick our a save of our image file as before. 
