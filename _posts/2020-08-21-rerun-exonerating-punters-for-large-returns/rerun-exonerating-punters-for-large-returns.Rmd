---
title: "RERUN: exonerating punters for large returns"
description: |
...
We notice two trends here. One is that in general, Net has increased over time, and quite a bit! This is actually a pretty persistent problem in puntalytics; we've dealt with it in various places, including in our EPA metric.
  
More pressing to our current analysis is that by the rough definitions we just set, coverage has also been improving consistently. We'll account for that by adjusting our "average" (or "expected") return length based on when that return happened! Thankfully, the `purrr` package, along with `tidyr::nest()`, make this pretty straightforward:
...
Now if we repeat the same analysis as above, we see that we've successfully era-adjusted our treatment of coverage:
...
### Evaluating punters with this new metric
...
<!-- This metric seems to make punters better across the board; can that be right? Let's check on 2018 and make sure it's not a fluke: -->

<!-- ```{r, echo=TRUE, layout="l-body", fig.width=7, fig.height=7} -->
<!-- mini18 <- mini %>% -->
<!--   filter(season==2018) -->

<!-- ggplot(mini18, mapping=aes(Net, RERUN, label = Name)) + -->
<!--   geom_abline() + -->
<!--   geom_image(aes(image = team_logo_espn)) + -->
<!--   geom_text_repel(size=4, point.padding = 0.6) + -->
<!--   annotate("text", x=42.5, y=38.5, label="Helped by coverage team", size=4, fontface = "bold") + -->
<!--   annotate("text", x=39.5, y=42.5, label="Hurt by coverage team", size=4, fontface = "bold") + -->
<!--   labs(title = "Were punters in 2018 helped or hurt by their coverage teams?",  -->
<!--        caption="figure @ThePuntRunts | data @nflfastR") + -->
<!--   theme_bw() -->
<!-- ``` -->

<!-- Interesting. Let's also take a look at this trend across the entire `nflfastR` era, in a more coarse-grained way: -->
